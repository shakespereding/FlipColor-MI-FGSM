...
class FixedSizeCrop(nn.Module):
    def __init__(self, size, fill=0, padding_mode="constant"):
        super().__init__()
        size = tuple(T._setup_size(
            size, error_msg="Please provide only two dimensions (h, w) for size."))
        self.crop_height = size[0]
        self.crop_width = size[1]
        # TODO: Fill is currently respected only on PIL. Apply tensor patch.
        self.fill = fill
    ...
    def _pad(self, img, target, padding):
        ...
        if target is not None:
            ...
            if "masks" in target:
                target["masks"] = F.pad(
                    target["masks"], padding, 0, "constant")
    ...
    def forward(self, img, target=None):
        ...
        if new_height != height or new_width != width:
            ...
            img, target = self._crop(
                img, target, top, left, new_height, new_width)
...
class RandomShortestSize(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.min_size = [min_size] if isinstance(
            min_size, int) else list(min_size)
    ...
    def forward(
        ...
        min_size = self.min_size[torch.randint(
            len(self.min_size), (1,)).item()]
...
def _copy_paste(
    ...
    if size1 != size2:
        paste_image = F.resize(
            paste_image, size1, interpolation=resize_interpolation)
    ...
    def forward(
        ...
        for target in targets:
            ...
            for k in ["masks", "boxes", "labels"]:
                torch._assert(
                    k in target, f"Key {k} should be present in targets")
                torch._assert(isinstance(
                    target[k], torch.Tensor), f"Value for the key {k} should be a tensor")
